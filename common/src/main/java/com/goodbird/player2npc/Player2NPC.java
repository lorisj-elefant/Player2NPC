...
public class Player2NPC {
    ...
    public static final ResourceLocation SPAWN_PACKET_ID = ResourceLocation.fromNamespaceAndPath("player2npc",
            "spawn_automatone");
    public static final ResourceLocation SPAWN_REQUEST_PACKET_ID = ResourceLocation.fromNamespaceAndPath("player2npc",
            "request_spawn_automatone");
    public static final ResourceLocation DESPAWN_REQUEST_PACKET_ID = ResourceLocation.fromNamespaceAndPath("player2npc",
            "request_despawn_automatone");
    ...
    public static RegistrySupplier<EntityType<AutomatoneEntity>> AUTOMATONE = ENTITY_TYPES.register(id("aicompanion"),
            () -> {
                ...
                EntityType<AutomatoneEntity> AUTOMATONE = EntityType.Builder
                        .<AutomatoneEntity>of(AutomatoneEntity::new, MobCategory.CREATURE)
                        .sized(EntityType.PLAYER.getWidth(), EntityType.PLAYER.getHeight()).clientTrackingRange(64)
                        .updateInterval(1).build(id("aicompanion").toString());
                LOGGER.info("CREATE");
                return AUTOMATONE;
            });
    ...
    public static void onInitialize() {
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, SPAWN_REQUEST_PACKET_ID,
                AutomatoneSpawnRequestPacket::handle);
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, DESPAWN_REQUEST_PACKET_ID,
                AutomatoneDespawnRequestPacket::handle);
        PlayerEvent.PLAYER_JOIN.register((player) -> CompanionManager.get(player).summonAllCompanionsAsync());
        PlayerEvent.PLAYER_QUIT.register((player) -> {
            LOGGER.info("Dismissing for player {} ", player.getName().getString());
